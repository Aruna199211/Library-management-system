...
-- return_status and branch m sare rows nhi h
use library_project;
select * from books;
...
Write a query to identify members who have overdue
 books (assume a 30-day return period). Display the
 member's_id, member's name, book title, issue date, 
 and days overdue.
 */
...
select * from issued_status;
select e.emp_name,b.*,count(i.issued_id) as no_of_book
from employee as e
join issued_status as i
...
select * from issued_status;
select * from return_status;
select m.member_name,b.book_title,count(r.book_quality) as damaged_book
from issued_status as i
join books as b
on b.isbn=i.issued_book_isbn
join members as m
...
-- Task 19: Stored Procedure Objective: Create a stored procedure to manage the status of 
...
--  first check if the book is available (status = 'yes'). If the book is available, 
...
begin
declare v_status varchar(5);  -- all the varianle
...
select m.member_id,
count(case when datediff(r.return_date,i.issued_date)>30 then 1 end) as overdue_duration_of_books,
sum(case when datediff(r.return_date,i.issued_date)>30
then (datediff(r.return_date,i.issued_date)-30)*0.50 else 0 end) as total_fine,
count(i.issued_id)as total_books_issued
from issued_status as i join members as m on i.issued_member_id=m.member_id
join return_status as r on i.issued_id=r.issued_id group by m.member_id;
